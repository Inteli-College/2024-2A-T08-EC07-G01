...
class KNR(BaseModel):
    ...
    UNIT: str = Field(..., description="Car UNIT")
    VALUE_ID: str = Field(..., description="Car VALUE_ID")
    VALUE: str = Field(..., description="Car VALUE")
    ...
    predicted_fail: Optional[List[str]] = Field("", description="Predicted fail")
    predicted_fail_code: Optional[List[int]] = Field(
        -1, description="Predicted fail code"
    )
    indicated_test: Optional[List[str]] = Field("", description="Indicated test")
    real_fail: Optional[List[str]] = Field("", description="Real fail")
    real_fail_code: Optional[List[int]] = Field(-1, description="Real fail code")
